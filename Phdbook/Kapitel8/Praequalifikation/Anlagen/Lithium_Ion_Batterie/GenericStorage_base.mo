...
model GenericStorage_base "Partial class for highly adaptable but non-physical model for all kinds of energy storages"
...
//________________________________________________________________________________//
  ...
  // _____________________________________________
  parameter TransiEnt.Storage.Base.GenericStorageParameters params "Record of generic storage parameters" annotation (choicesAllMatching, Placement(transformation(extent={{76,80},{94,98}}), iconTransformation(extent={{-116,-24},{-76,16}})));
  ...
  parameter Modelica.Units.SI.Time T_plant=params.T_plant "first order plant dynamic";
  ...
  TransiEnt.Basics.Interfaces.Electrical.ElectricPowerIn P_set "Grid side setpoint power (Positive = load request, negative = unload request)"
                                                                                                  annotation (Placement(transformation(extent={{-192,-22},{-152,18}}),iconTransformation(extent={{-192,-22},{-152,18}})));
  ...
  replaceable model StationaryLossModel =
      TransiEnt.Storage.Base.NoStationaryLoss constrainedby TransiEnt.Storage.Base.PartialStationaryLoss(final params=params)
...
ClaRa.Components.Utilities.Blocks.VariableGradientLimiter
  ...
  Modelica.Blocks.Sources.RealExpression P_max_load(y=params.P_max_load) annotation (Placement(transformation(extent={{-129,81},{-116,93}})));
  Modelica.Blocks.Sources.RealExpression P_max_unload_neg(y=-params.P_max_unload) annotation (Placement(transformation(extent={{-129,39},{-116,51}})));
  ...
  TransiEnt.Basics.Blocks.FirstOrder plantDynamic(Tau=T_plant, initOption=4) if use_plantDynamic annotation (Placement(transformation(extent={{76,-4},{84,4}})));
  ...
  Modelica.Blocks.Sources.RealExpression SOC1(y=SOC)             annotation (Placement(transformation(extent={{-168,58},{-144,78}})));
  Modelica.Blocks.Math.Product P_max_load_real annotation (Placement(transformation(extent={{-98,72},{-84,86}})));
  Modelica.Blocks.Math.Product P_max_unload_real annotation (Placement(transformation(extent={{-98,44},{-84,58}})));
  Modelica.Blocks.Logical.GreaterEqualThreshold isStorageFull(threshold=params.E_max) annotation (Placement(transformation(extent={{-32,-12},{-19,2}})));
...
equation
  ...
  connect(P_is_external.y, P_is_internal.u1) annotation (Line(points={{68.8,-13},{74,-13},{74,-30},{64,-30},{64,-50.8},{74.6,-50.8}},
                                                                                                                    color={0,0,127}));
  connect(PowerLimit.u, P_set) annotation (Line(points={{-72,66},{-80,66},{-80,-2},{-172,-2}},         color={0,0,127}));
  ...
  connect(SOC1.y, batteryPowerLimit.SOC) annotation (Line(points={{-142.8,68},{-136.4,68}}, color={0,0,127}));
  connect(P_max_load.y, P_max_load_real.u1) annotation (Line(points={{-115.35,87},{-107.675,87},{-107.675,83.2},{-99.4,83.2}}, color={0,0,127}));
  connect(batteryPowerLimit.P_max_load_star, P_max_load_real.u2) annotation (Line(
      points={{-114.8,71.6},{-107.4,71.6},{-107.4,74.8},{-99.4,74.8}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(P_max_load_real.y, PowerLimit.limit1) annotation (Line(points={{-83.3,79},{-77.65,79},{-77.65,74},{-72,74}}, color={0,0,127}));
  connect(P_max_unload_neg.y, P_max_unload_real.u2) annotation (Line(points={{-115.35,45},{-106.675,45},{-106.675,46.8},{-99.4,46.8}}, color={0,0,127}));
  connect(batteryPowerLimit.P_max_unload_star, P_max_unload_real.u1) annotation (Line(
      points={{-114.6,63.6},{-107.3,63.6},{-107.3,55.2},{-99.4,55.2}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(P_max_unload_real.y, PowerLimit.limit2) annotation (Line(points={{-83.3,51},{-78.65,51},{-78.65,58},{-72,58}}, color={0,0,127}));
  connect(E_is.y,isStorageEmpty. u) annotation (Line(points={{-46,-20},{-42,-20},{-42,-28.5},{-34.3,-28.5}}, color={0,0,127}));
  connect(isStorageFull.y, FullAndCharging.u2) annotation (Line(points={{-18.35,-5},{-13.175,-5},{-13.175,-6.6},{-8.3,-6.6}}, color={255,0,255}));
  connect(isStorageEmpty.y, EmptyAndDischarging.u2) annotation (Line(points={{-19.35,-28.5},{-14.675,-28.5},{-14.675,-28.6},{-9.3,-28.6}}, color={255,0,255}));
  connect(isStorageFull.u, E_is.y) annotation (Line(points={{-33.3,-5},{-33.3,-12.5},{-46,-12.5},{-46,-20}}, color={0,0,127}));
