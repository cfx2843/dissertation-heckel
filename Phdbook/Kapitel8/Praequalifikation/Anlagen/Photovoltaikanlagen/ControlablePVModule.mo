...
model ControlablePVModule "Simple efficiency-based PV model"
...
//________________________________________________________________________________//
  ...
  //Skymodel
  parameter Modelica.Units.SI.Angle longitude_local=Modelica.Units.Conversions.from_deg(10) "longitude of the local position, east positive, 10 East for Hamburg" annotation (Dialog(tab="Irradiance", group="Solartime"));
  parameter Modelica.Units.SI.Angle longitude_standard=Modelica.Units.Conversions.from_deg(15) "needed for calculation of coordinated universal time (utc), 15 for central european time, 30 for central european summer time" annotation (Dialog(tab="Irradiance", group="Solartime"));
  ...
  parameter Modelica.Units.SI.Angle latitude=Modelica.Units.Conversions.from_deg(53.55) "latitude of the local position, north posiive, 53,55 North for Hamburg" annotation (Dialog(tab="Irradiance", group="Extraterrestrial Irradiance"));
  parameter Modelica.Units.SI.Angle slope=Modelica.Units.Conversions.from_deg(30) "slope of the tilted surface, assumption" annotation (Dialog(tab="Irradiance", group="Extraterrestrial Irradiance"));
  parameter Modelica.Units.SI.Angle surfaceAzimuthAngle=0 "surface azimuth angle" annotation (Dialog(tab="Irradiance", group="Extraterrestrial Irradiance"));
  ...
  Modelica.Units.SI.Power POA_Irradiation(min=0) "plane of array irradiation usable for PV generation";
...
public
  ...
  TransiEnt.Basics.Interfaces.Electrical.ElectricPowerIn P_set annotation (Placement(transformation(
        extent={{-23,-28},{23,28}},
        rotation=90,
        origin={74,-117}), iconTransformation(
        extent={{-23,-28},{23,28}},
        rotation=270,
        origin={-26,-105})));
  TransiEnt.Basics.Interfaces.Electrical.ElectricPowerOut MaxP_out annotation (Placement(transformation(extent={{92,64},{132,104}}),  iconTransformation(extent={{92,64},{132,104}})));
equation
  // _____________________________________________
  //
