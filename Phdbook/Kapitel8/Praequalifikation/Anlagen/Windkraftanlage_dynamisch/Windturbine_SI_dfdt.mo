...
//________________________________________________________________________________//
  ...
  parameter Modelica.Units.SI.Time Ti_p=12 "Integral time constant of controller" annotation (Dialog(tab="Controller", group="Pitch"));
  parameter Modelica.Units.SI.Time Td_p=3 "Derivative time constant of controller" annotation (Dialog(tab="Controller", group="Pitch"));
...
//                                            /(Rotor.rho*Modelica.Constants.pi*Rotor.D^2*Rotor.cp_opt))^(1/3);
  final parameter Modelica.Units.SI.AngularVelocity omega_n=Rotor.lambda_opt*operationRanges.v_fullLoad*2/Rotor.D;
...
public
  TransiEnt.Components.Electrical.Machines.VariableSpeedActivePowerGenerator Generator(redeclare TransiEnt.Components.Boundaries.Electrical.ActivePower.Power terminal(change_sign=true) "P-Boundary for ActivePowerPort", changeSign=false)
                                                                                                                                    annotation (Placement(transformation(extent={{16,-32},{44,-2}})));
  WindturbineRotor Rotor(
    P_el_n=P_el_n,
  ...
  TransiEnt.Producer.Electrical.Wind.Controller.PitchController_SI_dt_df pitchController(
    ...
    yMax=yMax_p,
    yMin=yMin_p,
    controllerTypePitchCtrl=Modelica.Blocks.Types.SimpleController.P,
    v_wind=v_wind1.y,
    PitchControllerTimeConstant(T=20),
    Td=Td_p,
    beta_start=beta_start,
  ...
  Real der_f_grid;
  Real T_pu;
  Modelica.Units.SI.KineticEnergy E_rot;
  ...
  TransiEnt.Basics.Interfaces.Electrical.ElectricPowerIn P_el_set annotation (Placement(transformation(extent={{-118,14},{-84,48}}), iconTransformation(extent={{-118,14},{-84,48}})));
equation
  // _____________________________________________
  //
  //             Equations
  ...
  connect(pitchController.u_s, P_el_set) annotation (Line(points={{-62,7.2},{-80,7.2},{-80,31},{-101,31}}, color={0,0,127}));
    annotation (
