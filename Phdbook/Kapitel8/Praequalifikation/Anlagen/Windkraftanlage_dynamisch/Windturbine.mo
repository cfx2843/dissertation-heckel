...
//________________________________________________________________________________//
  ...
  parameter Modelica.Units.SI.Velocity v_wind_small=0.1 "Wind velocity considered as negligible" annotation (Dialog(tab="Wind Turbine Data"));
  parameter Real J = 12e6 "Moment of inertia of entire plant" annotation (Dialog(tab="Wind Turbine Data"));
  final parameter Modelica.Units.SI.Length D_Rotor=Rotor.D "Rotor diameter of wind turbine" annotation (Dialog(tab="Wind Turbine Data"));
  ...
  parameter Modelica.Units.SI.Time Ti_p=14 "Integral time constant of controller" annotation (Dialog(tab="Controller", group="Pitch"));
  parameter Modelica.Units.SI.Time Td_p=3.5 "Derivative time constant of controller" annotation (Dialog(tab="Controller", group="Pitch"));
  ...
  parameter Modelica.Units.SI.Velocity v_wind_start=10 "Setpoint for wind speed" annotation (Dialog(tab="Wind Turbine Data"));
  ...
  final parameter Modelica.Units.SI.AngularVelocity omega_n=Rotor.lambda_opt*v_fullload*2/Rotor.D "Nominal angular velocity of rotor";
  final parameter Modelica.Units.SI.AngularVelocity omega_start=if v_wind_start > 12 then omega_n else if (v_wind_start > 4 and v_wind_start < 12) then (Rotor.lambda_opt/Rotor.D*2*v_wind_start) else 1e-3;
...
public
 ...
 TransiEnt.Components.Electrical.Machines.VariableSpeedActivePowerGenerator Generator(changeSign=true)  annotation (Dialog(group="Replaceable Components"),Placement(transformation(extent={{18,-22},{46,8}})));
  ...
  Modelica.Blocks.Sources.RealExpression P_is(y=P_el_is)    annotation (Placement(transformation(extent={{-100,52},{-84,72}})));

  TorqueController_modified torqueController_modified(
    T_torqueControl=3,
    v_wind=v_wind1.y,
 ...
 TransiEnt.Components.Electrical.Machines.ExcitationSystemsVoltageController.DummyExcitationSystem Exciter constrainedby TransiEnt.Components.Electrical.Machines.ExcitationSystemsVoltageController.PartialExcitationSystem
  ...
  TransiEnt.Basics.Interfaces.Electrical.ElectricPowerIn P_el_set annotation (Placement(transformation(extent={{-120,-106},{-82,-72}}), iconTransformation(extent={{-120,-106},{-82,-72}})));
equation
  // _____________________________________________
  //
  ...
  connect(omega_is.y, torqueController_modified.omega_is) annotation (Line(points={{-0.6,55},{4,55},{4,59.4545},{12,59.4545}}, color={0,0,127}));

  connect(P_is.y, pitchController.u_m) annotation (Line(points={{-83.2,62},{-68,62}},
                                  color={0,0,127}));
  connect(wind_fullload.y, torqueController_modified.wind_fullload) annotation (Line(points={{-0.6,71},{8,71},{8,70.3636},{12,70.3636}}, color={0,0,127}));
  ...
  connect(torqueController_modified.y, Generator.tau_set) annotation (Line(points={{33,64.9091},{33,64.9091},{42,64.9091},{42,28},{18.98,28},{18.98,-16.75}}, color={0,0,127}));
  ...
  connect(pitchController.u_s, P_el_set) annotation (Line(points={{-68,48},{-78,48},{-78,-89},{-101,-89}}, color={0,0,127}));
  connect(torqueController_modified.P_el_set, P_el_set) annotation (Line(points={{11.9,56.8182},{11.9,-15.5909},{-101,-15.5909},{-101,-89}}, color={0,127,127}));
    annotation (
