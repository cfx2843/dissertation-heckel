...
partial model PartialEngine_idealGas_static "Base class for engines with necessary connectors"
...
//________________________________________________________________________________//
  ...
  // _____________________________________________
  final parameter Modelica.Units.SI.HeatFlowRate Q_flow_out_max=Specification.Q_flow_out_max "Maximum heat flow rate in W" annotation (Dialog(tab="Technical Specifications"));
  final parameter Modelica.Units.SI.HeatFlowRate Q_flow_out_min=Specification.Q_flow_out_min "Minimum heat flow rate in W" annotation (Dialog(tab="Technical Specifications"));
  final parameter Modelica.Units.SI.Power P_el_max=Specification.P_el_max "Maximum power output in W" annotation (Dialog(tab="Technical Specifications"));
  final parameter Modelica.Units.SI.Power P_el_min=Specification.P_el_min "Minimum power output in W" annotation (Dialog(tab="Technical Specifications"));

  //Engine parameters
  final parameter Modelica.Units.SI.Volume engineDisplacement=Specification.engineDisplacement;
  ...
  final parameter Modelica.Units.SI.AngularVelocity omega_n=Specification.omega_n;
  final parameter Modelica.Units.SI.CoefficientOfHeatTransfer k=Specification.k;
  final parameter Modelica.Units.SI.Area A=Specification.width*Specification.height*2 + Specification.length*Specification.height*2 + Specification.width*Specification.length;
  final parameter Modelica.Units.SI.Mass m_engine=Specification.m_engine "Approximate mass of motor" annotation (Dialog(tab="Technical Specifications"));
  final parameter Modelica.Units.SI.ThermalConductivity thermalConductivity=Specification.thermalConductivity "Thermal conductivity between core and mantle";

  //Efficiencies
  final parameter Modelica.Units.SI.Efficiency eta_el_max=Specification.eta_el_max "Electrical efficiency" annotation (Dialog(tab="Technical Specifications"));
  final parameter Modelica.Units.SI.Efficiency eta_el_min=Specification.eta_el_min "Electrical efficiency at P_el_min" annotation (Dialog(tab="Technical Specifications"));
  final parameter Modelica.Units.SI.Efficiency eta_h_max=Specification.eta_h_max "Total efficiency at P_el_min" annotation (Dialog(tab="Technical Specifications"));
  final parameter Modelica.Units.SI.Efficiency eta_h_min=Specification.eta_h_min "Total efficiency at P_el_max" annotation (Dialog(tab="Technical Specifications"));

  //    //Temperatures
  final parameter Modelica.Units.SI.Temperature T_opt=Specification.T_opt "Optimal temperature for maximum efficiency";
  parameter Modelica.Units.SI.Temperature T_site=290 "Average ambient temperature at plant site" annotation (Dialog(group="Standby losses"));
...
//parameter TransiEnt.Producer.Combined.SmallScaleCHP.Base.PartloadEfficiency.PartloadEfficiencyCharacteristic EfficiencyCharLine=TransiEnt.Producer.Combined.SmallScaleCHP.Base.PartloadEfficiency.ConstantEfficiency() "choose characteristic efficiency line" annotation(Dialog(group="Physical Constraints"), choicesAllMatching=true);
  ...
  Modelica.Units.SI.HeatFlowRate Q_flow_fuel(start=1) "Heat flow rate input from fuel";
...
end PartialEngine_idealGas_static;
