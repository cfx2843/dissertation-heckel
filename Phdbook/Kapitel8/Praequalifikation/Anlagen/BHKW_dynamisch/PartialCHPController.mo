...
//________________________________________________________________________________//
  ...
  parameter Modelica.Units.SI.Time t_OnOff=1800 "Minimum time between switching";
  parameter Boolean useGridTemperatures = false "Whether or not to use target grid temperatures";
  parameter Modelica.Units.SI.Temperature T_supply_const=363.15 "Target temperature of CHP" annotation (Dialog(enable=not useGridTemperatures));
  parameter Modelica.Units.SI.Temperature T_return_max=Specification.T_return_max "Maximum allowed return temperature";
  ...
  Modelica.Units.SI.Time runningTime(start=0) "Total time running";
  Modelica.Units.SI.Time startTime(start=0) "Time since last start";
  Modelica.Units.SI.Time stopTime(start=-2*t_OnOff) "Time since last stop" annotation (Dialog(group="Initialization", showStartAttribute=true));
  Integer startCounter(start=0) "Counter to start on/off cycles";
  Modelica.Units.SI.TemperatureDifference dT "Target temperature difference";
  Modelica.Units.SI.Pressure dp "Measured pressure difference in CHP";
...
<p>Model created by Pascal Dubucq (dubucq@tuhh.de), Aug 2014</p>
</html>"));
