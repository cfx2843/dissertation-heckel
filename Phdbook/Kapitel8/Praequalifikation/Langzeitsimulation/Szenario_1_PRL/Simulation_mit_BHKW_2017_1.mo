...
equation
 // prognosegenauigkeit.PowerIn=-ElectricGrid.epp.P;
  ...
  connect(bHKW_statisch.epp, ElectricGrid.epp) annotation (Line(
      points={{82,-175.12},{82,-175.34},{178,-175.34},{178,206}},
      color={0,135,135},
      thickness=0.5));
  ...
  connect(Zuverlaessigkeit.Erfuellt, regler_Virtuelles_Kraftwerk.PRL_erfuellt) annotation (Line(points={{-147.2,18.8},{-52.6,18.8},{-52.6,17.84},{-5.46,17.84}},color={255,0,255}));
  connect(Summe_Leistung_gesetzt.y, Energie_gesetzt.u) annotation (Line(points={{-119.02,-52},{-136,-52},{-136,-54},{-146,-54}}, color={0,0,127}));
  ...
  connect(Energie_gesetzt_BHKW.u, bHKW_statisch.electricPowerIn) annotation (Line(points={{-156,-210},{-68,-210},{-68,-180},{-60,-180},{-60,-211},{-9.22667,-211}}, color={0,0,127}));
  connect(greaterThreshold.y, Zuverlaessigkeit_Waerme.Erfuellt) annotation (Line(points={{-7,-256},{-12,-256},{-12,-249.2},{-157.2,-249.2}},color={255,0,255}));
  ...
  connect(Prognose_Optimierung_S1.y[1], G.u) annotation (Line(points={{322,-175},{316,-175},{316,-142},{318,-142}}, color={0,0,127}));
  connect(Energie_gesetzt1.u, regler_Virtuelles_Kraftwerk.P_sum_AP) annotation (Line(points={{-144,-144},{-76,-144},{-76,-145.84},{-2.34,-145.84}}, color={0,0,127}));
  connect(Energie_gesetzt.y,Einspeisung)  annotation (Line(points={{-169,-54},{-226,-54},{-226,-50},{-250,-50}}, color={0,0,127}));
  connect(Energie_gesetzt_BHKW.y,Energie_BHKW)  annotation (Line(points={{-179,-210},{-250,-210}},color={0,0,127}));
  connect(Zuverlaessigkeit.Percentage,Sicherheit)  annotation (Line(
      points={{-169.2,13},{-191.6,13},{-191.6,12},{-212,12}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Zuverlaessigkeit_Waerme.Percentage,Sicherheit_Waerme)  annotation (Line(
      points={{-179.2,-255},{-220.6,-255},{-220.6,-268},{-270,-268}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Energie_gesetzt1.y, Spotmarktangebot) annotation (Line(points={{-167,-144},{-244,-144},{-244,-146},{-242,-146}}, color={0,0,127}));
  annotation (
