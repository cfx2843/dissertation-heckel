...
model Simulation_mit_REVeLY_2016_1
  ...
  Modelica.Blocks.Interfaces.RealOutput Sicherheit annotation (Placement(transformation(
        ...
        origin={-176,8})));
public
  parameter Modelica.Units.SI.Power P_el_N=5.6514e6 "Nennleistung Elektrolyseur";
  parameter Real varG=0.4 "G";
  parameter Modelica.Units.SI.Mass m_H2=1167.4758 "Speichermasse Wasserstoff";
  parameter Real SOC_min=0.0789 "Sicherheitspuffer";
...
equation
 // prognosegenauigkeit.PowerIn=-ElectricGrid.epp.P;
  ...
  connect(P_AP.y, Spotmarktangebot) annotation (Line(points={{-59,-150},{-158,-150},{-158,-152},{-218,-152}},
                                                                                      color={0,0,127}));
  connect(Energie_Brennstoffzelle.y, Energie_Brennstoff) annotation (Line(points={{-59,-184},{-158,-184},{-158,-186},{-218,-186}},
                                                                                                           color={0,0,127}));
  connect(zyklanzahl_von_SOC.Zyklen, Zyklenzahl) annotation (Line(points={{-60.2,-220},{-216,-220}},                       color={0,0,127}));
  connect(anteil_Fehler.Erfuellt, Zuverlaessigkeit.Erfuellt) annotation (Line(points={{-131.2,58.8},{-108,58.8},{-108,12.8},{-127.2,12.8}}, color={255,0,255}));
  connect(anteil_Fehler.Soc_Hoeher, greaterEqualThreshold.y) annotation (Line(points={{-131.2,52},{-126,52},{-126,40},{-121,40}}, color={255,0,255}));
  connect(rev_PEM_FC_statisch_SRL.SOC, greaterEqualThreshold.u) annotation (Line(points={{50.4,-173.4},{-82,-173.4},{-82,40},{-98,40}}, color={0,0,127}));
  connect(lessEqualThreshold.y, anteil_Fehler.Soc_Niedriger) annotation (Line(points={{-119,74},{-124,74},{-124,67.2},{-131.2,67.2}}, color={255,0,255}));
  connect(lessEqualThreshold.u, greaterEqualThreshold.u) annotation (Line(points={{-96,74},{-96,40},{-98,40}}, color={0,0,127}));
  annotation (
