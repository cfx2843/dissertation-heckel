...
model Simulation_mit_LiIon_2016_2
...
protected
  ...
  Phdbook.Kapitel8.Praequalifikation.Anlagen.Lithium_Ion_Batterie.Zyklanzahl_von_SOC zyklanzahl_von_SOC(start=0.5) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-164,-202})));
  TransiEnt.Basics.Tables.GenericDataTable Daten_Optimierung_S1(
    multiple_outputs(
      quantity=4,
      start=false,
      fixed=true) = true,
...
equation
 // prognosegenauigkeit.PowerIn=-ElectricGrid.epp.P;
  ...
  connect(G.y,primaryBalancingController. PRL_Angebot) annotation (Line(points={{142,-177},{142,-110.3},{129.3,-110.3}},
                                                                                                                      color={0,0,127}));
  ...
  connect(Data_5.y[4],Wind_5. v_wind) annotation (Line(points={{39,114},{40,114},{40,113.8},{47.6,113.8}}, color={0,0,127}));
  ...
  connect(regler_Virtuelles_Kraftwerk_mitLiIon.PRL_Angebot, G.y) annotation (Line(points={{78.78,-105.067},{118.39,-105.067},{118.39,-177},{142,-177}}, color={0,127,127}));
  connect(regler_Virtuelles_Kraftwerk_mitLiIon.PRL_erfuellt, Zuverlaessigkeit.Erfuellt) annotation (Line(points={{-4.68,13.1048},{-16.78,13.1048},{-16.78,12.8},{-145.2,12.8}}, color={255,0,255}));
  ...
  connect(regler_Virtuelles_Kraftwerk_mitLiIon.Max_Wind_2, Wind_3.MaxP_Out) annotation (Line(points={{80.34,-16.2286},{100,-16.2286},{100,154.6},{66.6,154.6}},
                                                                                                                                                          color={0,127,127}));
  ...
  connect(regler_Virtuelles_Kraftwerk_mitLiIon.P_set_Wind_2, Wind_3.P_set) annotation (Line(
      points={{-3.12,-24.6095},{-30,-24.6095},{-30,140.3},{45.2,140.3}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  ...
  connect(regler_Virtuelles_Kraftwerk_mitLiIon.P_set_PV_1, Summe_Leistung_gesetzt.u[10]) annotation (Line(
      ...
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Energie_gesetzt.u, Summe_Leistung_gesetzt.y) annotation (Line(points={{-144,-32},{-129.02,-32}}, color={0,0,127}));
  connect(zyklanzahl_von_SOC.SOC, liIon_Battery.SOC) annotation (Line(points={{-153.1,-201.9},{50,-201.9},{50,-202},{54,-202},{54,-183.4}},
                                                                                                                                          color={0,0,127}));
  connect(Energie_gesetzt1.u, regler_Virtuelles_Kraftwerk_mitLiIon.P_sum_AP) annotation (Line(points={{-144,-146},{-82,-146},{-82,-149.486},{-1.56,-149.486}}, color={0,0,127}));
  connect(Energie_gesetzt.y, Einspeisung) annotation (Line(points={{-167,-32},{-158,-32},{-158,-34},{-194,-34}}, color={0,0,127}));
  connect(Energie_gesetzt1.y, Spotmarktangebot) annotation (Line(points={{-167,-146},{-178,-146},{-178,-150},{-190,-150}}, color={0,0,127}));
  connect(zyklanzahl_von_SOC.Zyklen, Zyklenzahl) annotation (Line(points={{-176.2,-202},{-150,-202},{-150,-208},{-176,-208}}, color={0,0,127}));
  connect(Zuverlaessigkeit.Percentage, Sicherheit) annotation (Line(
      points={{-167.2,7},{-158.6,7},{-158.6,2},{-190,2}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Daten_Optimierung_S1.y[1], G.u) annotation (Line(points={{138,-221},{138,-200},{142,-200}}, color={0,0,127}));
  annotation (
...
end Simulation_mit_LiIon_2016_2;
