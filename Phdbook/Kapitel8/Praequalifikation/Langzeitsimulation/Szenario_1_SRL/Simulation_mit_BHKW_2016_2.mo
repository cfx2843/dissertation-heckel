...
equation
 // prognosegenauigkeit.PowerIn=-ElectricGrid.epp.P;
  ...
  connect(regler_Virtuelles_Kraftwerk.P_sum_AP, Energie_Spotmarkt.u) annotation (Line(
      ...
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Energie_gesetzt.y,Einspeisung)  annotation (Line(points={{-177,-52},{-210,-52}},                       color={0,0,127}));
  connect(Energie_Spotmarkt.y,Spotmarktangebot)  annotation (Line(points={{-53,-154},{-120,-154}}, color={0,0,127}));
  connect(Energie_gesetzt_BHKW.y,Energie_BHKW)  annotation (Line(points={{-97,-182},{-156,-182},{-156,-180},{-152,-180}},
                                                                                                  color={0,0,127}));
  connect(Zuverlaessigkeit.Percentage,Sicherheit)  annotation (Line(
      points={{-121.2,13},{-193.6,13},{-193.6,6},{-212,6}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Zuverlaessigkeit_Waerme.Percentage,Sicherheit_Waerme)  annotation (Line(
      points={{-39.2,-267},{-60.6,-267},{-60.6,-266},{-76,-266}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  annotation (
