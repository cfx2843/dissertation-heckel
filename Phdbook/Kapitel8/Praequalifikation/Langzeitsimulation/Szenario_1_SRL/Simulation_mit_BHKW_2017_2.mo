...
protected
  ...
  Phdbook.Kapitel8.Praequalifikation.Anlagen.Photovoltaikanlagen.ControlablePVModule PV_18(P_inst(displayUnit="MW") = 2315400, use_input_data=true) annotation (Placement(transformation(extent={{50,228},{70,248}})));
...
equation
 // prognosegenauigkeit.PowerIn=-ElectricGrid.epp.P;
  ...
  connect(Wind_3.v_wind,Data_3. y[4]) annotation (Line(points={{47.6,147.8},{44,147.8},{44,146},{37,146}}, color={0,0,127}));
  ...
  connect(regler_Virtuelles_Kraftwerk.P_sum_AP, Energie_Spotmarkt.u) annotation (Line(
      ...
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Energie_gesetzt.y,Einspeisung)  annotation (Line(points={{-177,-52},{-202,-52}},                       color={0,0,127}));
  connect(Energie_Spotmarkt.y,Spotmarktangebot)  annotation (Line(points={{-53,-154},{-86,-154}},  color={0,0,127}));
  connect(Energie_gesetzt_BHKW.y,Energie_BHKW)  annotation (Line(points={{-97,-182},{-120,-182}}, color={0,0,127}));
  connect(Zuverlaessigkeit.Percentage,Sicherheit)  annotation (Line(
      points={{-121.2,13},{-129.6,13},{-129.6,12},{-148,12}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Zuverlaessigkeit_Waerme.Percentage,Sicherheit_Waerme)  annotation (Line(
      points={{-39.2,-267},{-44.6,-267},{-44.6,-266},{-64,-266}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Einspeisung, Einspeisung) annotation (Line(points={{-202,-52},{-202,-52}}, color={0,0,127}));
  annotation (
