...
protected
  ...
  Modelica.Blocks.Continuous.Integrator Energie_Spotmarkt annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-62,-146})));
  TransiEnt.Basics.Tables.GenericDataTable Prognose_Optimierung_S2(
    multiple_outputs(
      quantity=4,
      start=false,
      fixed=true) = true,
...
equation
 // prognosegenauigkeit.PowerIn=-ElectricGrid.epp.P;
  ...
  connect(Wind_1.v_wind,Data_1. y[4]) annotation (Line(points={{47.6,177.8},{42,177.8},{42,178},{35,178}}, color={0,0,127}));
  ...
  connect(regler_Virtuelles_Kraftwerk_mitLiIon.P_sum_AP, Energie_Spotmarkt.u) annotation (Line(
      points={{-1.56,-149.486},{-25.78,-149.486},{-25.78,-146},{-50,-146}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Prognose_Optimierung_S2.y[1], G.u) annotation (Line(points={{116,-215},{118,-215},{118,-196}}, color={0,0,127}));
  connect(Abrufanteil_SRL.y[1], P_SRL.u2) annotation (Line(points={{187,-88},{168,-88},{168,-86},{146,-86}}, color={0,0,127}));
  connect(zyklanzahl_von_SOC.Zyklen, Zyklenzahl) annotation (Line(points={{27.8,-202},{6,-202},{6,-286},{44,-286}}, color={0,0,127}));
  connect(Energie_Spotmarkt.y, Spotmarktangebot) annotation (Line(points={{-73,-146},{-90,-146},{-90,-150},{-106,-150}}, color={0,0,127}));
  connect(Energie_gesetzt.y, Einspeisung) annotation (Line(points={{-187,-32},{-200,-32},{-200,-30},{-216,-30}}, color={0,0,127}));
  connect(Zuverlaessigkeit.Percentage,Sicherheit)  annotation (Line(
      points={{-185.2,29},{-199.6,29},{-199.6,28},{-218,28}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  annotation (
