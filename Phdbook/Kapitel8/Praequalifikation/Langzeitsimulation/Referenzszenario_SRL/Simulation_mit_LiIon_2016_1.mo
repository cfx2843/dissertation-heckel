within Phdbook.Kapitel8.Praequalifikation.Langzeitsimulation.Referenzszenario_SRL;
model Simulation_mit_LiIon_2016_1
  ...
  Langzeitsimulation.Messung.Anteil_Fehler anteil_Fehler annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-170,92})));
  Modelica.Blocks.Logical.GreaterEqualThreshold greaterEqualThreshold(threshold=0.95) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={-138,72})));
  Modelica.Blocks.Logical.LessEqualThreshold lessEqualThreshold(threshold=SOC_min) annotation (Placement(transformation(
        ...
        origin={-136,106})));
public
  parameter Real varG=0.4;
  parameter Modelica.Units.SI.Power P_B=5.6514e6 "Batterieleistung";
  parameter Real E_B=19.2615 "Batteriekapazität in kWh";
  parameter Real SOC_min=0.0786 "Mindestfüllstand Batterie";
  ...
  Langzeitsimulation.Regler.Regler_Virtuelles_Kraftwerk_mitSpeicher_rev_FC regler_Virtuelles_Kraftwerk_mitLiIon(SOC_min=SOC_min, SOC_max=0.99) annotation (Placement(transformation(
        extent={{-39,-88},{39,88}},
        rotation=180,
        origin={39,-64})));
...
equation
 // prognosegenauigkeit.PowerIn=-ElectricGrid.epp.P;
  ...
  connect(zyklanzahl_von_SOC.SOC, liIon_Battery.SOC) annotation (Line(points={{50.9,-201.9},{50,-201.9},{50,-202},{54,-202},{54,-183.4}}, color={0,0,127}));
  ...
  connect(G.y, P_SRL.u1) annotation (Line(points={{118,-173},{140,-173},{140,-114},{146,-114},{146,-98}}, color={0,0,127}));
  connect(regler_Virtuelles_Kraftwerk_mitLiIon.P_sum_AP, Energie_Spotmarkt.u) annotation (Line(
      points={{-1.56,-149.486},{-25.78,-149.486},{-25.78,-150},{-48,-150}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  ...
  connect(Zuverlaessigkeit.Percentage,Sicherheit)  annotation (Line(
      points={{-185.2,29},{-195.6,29},{-195.6,28},{-214,28}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Energie_gesetzt.y, Einspeisung) annotation (Line(points={{-187,-32},{-196,-32},{-196,-30},{-212,-30}}, color={0,0,127}));
  connect(Energie_Spotmarkt.y, Spotmarktangebot) annotation (Line(points={{-71,-150},{-102,-150}}, color={0,0,127}));
  connect(zyklanzahl_von_SOC.Zyklen, Zyklenzahl) annotation (Line(points={{27.8,-202},{10,-202},{10,-286},{48,-286}}, color={0,0,127}));
  connect(anteil_Fehler.Erfuellt, Zuverlaessigkeit.Erfuellt) annotation (Line(points={{-159.2,90.8},{-136,90.8},{-136,34.8},{-163.2,34.8}}, color={255,0,255}));
  connect(anteil_Fehler.Soc_Hoeher,greaterEqualThreshold. y) annotation (Line(points={{-159.2,84},{-154,84},{-154,72},{-149,72}}, color={255,0,255}));
  connect(lessEqualThreshold.y,anteil_Fehler. Soc_Niedriger) annotation (Line(points={{-147,106},{-152,106},{-152,99.2},{-159.2,99.2}},
                                                                                                                                      color={255,0,255}));
  connect(liIon_Battery.SOC, greaterEqualThreshold.u) annotation (Line(points={{54,-183.4},{64,-183.4},{64,-182},{66,-182},{66,-244},{-98,-244},{-98,72},{-126,72}}, color={0,0,127}));
  connect(lessEqualThreshold.u, greaterEqualThreshold.u) annotation (Line(points={{-124,106},{-110,106},{-110,72},{-126,72}}, color={0,0,127}));
  annotation (
...
end Simulation_mit_LiIon_2016_1;
