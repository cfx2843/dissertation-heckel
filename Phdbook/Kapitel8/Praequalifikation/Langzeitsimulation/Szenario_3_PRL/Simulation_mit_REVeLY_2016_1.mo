...
model Simulation_mit_REVeLY_2016_1
  ...
  Modelica.Blocks.Interfaces.RealOutput Sicherheit annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-268,30})));
  Langzeitsimulation.Messung.Anteil_Fehler anteil_Fehler annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={-196,82})));
  Modelica.Blocks.Logical.GreaterEqualThreshold greaterEqualThreshold(threshold=0.99) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={-164,62})));
  Modelica.Blocks.Logical.LessEqualThreshold lessEqualThreshold(threshold=SOC_min) annotation (Placement(transformation(
        ...
        origin={-162,96})));
public
  parameter Modelica.Units.SI.Power P_el_N=3.6364e6 "Nennleistung Elektrolyseur";
  parameter Real varG=0.3 "G";
  parameter Modelica.Units.SI.Mass m_H2=153.6872 "Speichermasse Wasserstoff";
  parameter Real SOC_min=0.3722 "Sicherheitspuffer";
...
equation
 // prognosegenauigkeit.PowerIn=-ElectricGrid.epp.P;
  ...
  connect(regler_Virtuelles_Kraftwerk_mitLiIon.PRL_erfuellt, Zuverlaessigkeit.Erfuellt) annotation (Line(points={{-4.68,13.1048},{-16.78,13.1048},{-16.78,20.8},{-191.2,20.8}}, color={255,0,255}));
  ...
  connect(regler_Virtuelles_Kraftwerk_mitLiIon.P_set_PV_1, Summe_Leistung_gesetzt.u[10]) annotation (Line(
      ...
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Energie_gesetzt.u, Summe_Leistung_gesetzt.y) annotation (Line(points={{-166,-38},{-146,-38},{-146,-32},{-129.02,-32}},
                                                                                                           color={0,0,127}));
  ...
  connect(regler_Virtuelles_Kraftwerk_mitLiIon.P_set_LiIon, rev_PEM_FC_statisch.electricPowerIn) annotation (Line(
      ...
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(rev_PEM_FC_statisch.SOC, regler_Virtuelles_Kraftwerk_mitLiIon.SOC_LiIon) annotation (Line(points={{50.4,-173.4},{96,-173.4},{96,-144.457},{79.56,-144.457}}, color={0,0,127}));
  connect(zyklanzahl_von_SOC.SOC, regler_Virtuelles_Kraftwerk_mitLiIon.SOC_LiIon) annotation (Line(points={{-159.1,-213.9},{64,-213.9},{64,-173.4},{96,-173.4},{96,-144.457},{79.56,-144.457}},color={0,0,127}));
  connect(Energie_Brennstoffzelle.u, rev_PEM_FC_statisch.electricPowerIn) annotation (Line(points={{-166,-160},{-24,-160},{-24,-161.6},{29.2,-161.6}},color={0,0,127}));
  connect(P_AP.u, regler_Virtuelles_Kraftwerk_mitLiIon.P_sum_AP) annotation (Line(points={{-164,-110},{-16,-110},{-16,-149.486},{-1.56,-149.486}},
                                                                                                                                                 color={0,0,127}));
  connect(Prognose_Optimierung_S1.y[1], G.u) annotation (Line(points={{118,-227},{118,-196}}, color={0,0,127}));
  connect(Zuverlaessigkeit.Percentage,Sicherheit)  annotation (Line(
      points={{-213.2,15},{-253.6,15},{-253.6,30},{-268,30}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(Energie_gesetzt.y,Einspeisung)  annotation (Line(points={{-189,-38},{-240,-38},{-240,-10},{-282,-10}},
                                                                                           color={0,0,127}));
  connect(P_AP.y,Spotmarktangebot)  annotation (Line(points={{-187,-110},{-250,-110},{-250,-130},{-310,-130}},
                                                                                      color={0,0,127}));
  connect(Energie_Brennstoffzelle.y,Energie_Brennstoff)  annotation (Line(points={{-189,-160},{-250,-160},{-250,-164},{-310,-164}},
                                                                                                           color={0,0,127}));
  connect(zyklanzahl_von_SOC.Zyklen,Zyklenzahl)  annotation (Line(points={{-182.2,-214},{-252,-214},{-252,-198},{-308,-198}},
                                                                                                                           color={0,0,127}));
  connect(anteil_Fehler.Erfuellt, Zuverlaessigkeit.Erfuellt) annotation (Line(points={{-185.2,80.8},{-162,80.8},{-162,20.8},{-191.2,20.8}}, color={255,0,255}));
  connect(anteil_Fehler.Soc_Hoeher,greaterEqualThreshold. y) annotation (Line(points={{-185.2,74},{-180,74},{-180,62},{-175,62}}, color={255,0,255}));
  connect(lessEqualThreshold.y,anteil_Fehler. Soc_Niedriger) annotation (Line(points={{-173,96},{-178,96},{-178,89.2},{-185.2,89.2}}, color={255,0,255}));
  connect(rev_PEM_FC_statisch.SOC, greaterEqualThreshold.u) annotation (Line(points={{50.4,-173.4},{58,-173.4},{58,-188},{-96,-188},{-96,62},{-152,62}}, color={0,0,127}));
  connect(lessEqualThreshold.u, greaterEqualThreshold.u) annotation (Line(points={{-150,96},{-144,96},{-144,94},{-134,94},{-134,62},{-152,62}}, color={0,0,127}));
  annotation (
...
end Simulation_mit_REVeLY_2016_1;
