...
Consumers=["L1";"Lb";"Lc";"La"];     % Path to the heatPorts of the Consumers in the Modelica model
numberOfConsumers=3;                                                            % Number of Consumers for for-loops
...
rel_tol=0.01;                                                                    % relative Tolerance value for tolerance band
...
for k = 1:numberOfConsumers
    P_flow_0(k,:)=dym_getResult(res0,[char(Consumers(k)) '.epp.P'])*(-1);
    P_flow_1(k,:)=dym_getResult(res1,[char(Consumers(k)) '.epp.P'])*(-1);
end
...
Delta_x(k)=-1;
    ...
    Delta=(abs(P0(k,i)-P1(k,i)))-abs((rel_tol*P0(k,i)));
    if Delta>Delta_x(k)
        Delta_x(k)=Delta;
        h(k)=i;
    end
        ...
        j(k)= (find(abs(P0(k,:)-P1(k,:))==abs(Delta_x(k)+rel_tol*abs(Mean_P(k)))));     % Find Index where Delta x occurs
        ...
        for i = 514:length(P0)
            if abs(P0(k,i)-P1(k,i))>rel_tol*abs(P0(k,i))                     % Find Index where characteristic value first leaves tolerance band
                a=i;
                break
            end
            a=length(P0);
        ...
        for i = a:length(P0)                                                   % Find Index where characteristic value returns to tolerance band
            if abs(P0(k,i)-P1(k,i))<rel_tol*abs(P0(k,i))
                b=i;
                break
            end
            b=length(P0);
     ...
     %    Index(k,:) = [a;b];
        ...
        A(k)=trapz(t_(1,Index(k,1):Index(k,4)),max(((abs(P0(k,Index(k,1):Index(k,4))-P1(k,Index(k,1):Index(k,4))))-rel_tol*abs(P0(k,i))),0)/3600);  % Compute Integral
