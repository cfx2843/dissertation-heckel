...
[Pa,Qa,Ua,dta,konvfehlerbasis] = ConverttoPSTLFNR(Leitungen,Knoten,AnzL,AnzK);
...
%Berechnung der Leitungsflüsse auf den Leitungen
for i=1:AnzL
    if Leitungen(i,4)==380
        R=Leitungen(i,3)*Rb380;     %Wiederstand=Länge*Wiederstandsbelag
        X=Leitungen(i,3)*Xb380;     %Längsreaktanz=Länge*Wiederstandsbelag
        B=Leitungen(i,3)*Bb380;     %Quersuszeptanz=Länge*Wiederstandsbelag
    ...
    elseif Leitungen(i,4)==220
        R=Leitungen(i,3)*Rb220;
        X=Leitungen(i,3)*Xb220;
        B=Leitungen(i,3)*Bb220;
        yij = (inv(R+1i*X))*Leitungen(i,5)/ue^2;
        SLeitung(i)=(abs((Ua(Leitungen(i,1))*220000.*exp(1i*dta(Leitungen(i,1))))-(Ua(Leitungen(i,2))*220000.*exp(1i*dta(Leitungen(i,2)))))^2)*conj(yij);
    elseif Leitungen(i,4)==110
        R=Leitungen(i,3)*Rb110;
        X=Leitungen(i,3)*Xb110;
        B=Leitungen(i,3)*Bb110;
        yij = (inv(R+1i*X))*Leitungen(i,5)/ue2^2;
        SLeitung(i)=(abs((Ua(Leitungen(i,1))*110000*exp(1i*dta(Leitungen(i,1))))-(Ua(Leitungen(i,2))*110000.*exp(1i*dta(Leitungen(i,2)))))^2)*conj(yij);
    end
end
...
for zielAnz=30:5:125
   ...
   SLeitungencluster(loesch==1)=[];
    ...
    [Par,Qar,Uar,dtar,konvfehler] = ConverttoPSTLFNR(clusterLeitungen,clusterKnoten,AnzClusterLeitungen,AnzCluster);
...
for i=1:AnzClusterLeitungen
    if clusterLeitungen(i,4)==380
        R=clusterLeitungen(i,3)*Rb380;     %Wiederstand=Länge*Wiederstandsbelag
        X=clusterLeitungen(i,3)*Xb380;     %Längsreaktanz=Länge*Wiederstandsbelag
        B=clusterLeitungen(i,3)*Bb380;     %Quersuszeptanz=Länge*Wiederstandsbelag
        yij = (inv(R+1i*X))*clusterLeitungen(i,5);
        SLeitungr(i)=(abs((Uar(cluster(:,1)==clusterLeitungen(i,1))*380000.*exp(1i*dtar(cluster(:,1)==clusterLeitungen(i,1))))-(Uar(cluster(:,1)==clusterLeitungen(i,2))*380000.*exp(1i*dtar(cluster(:,1)==clusterLeitungen(i,2)))))^2)*conj(yij);
    elseif clusterLeitungen(i,4)==220
        R=clusterLeitungen(i,3)*Rb220;
        X=clusterLeitungen(i,3)*Xb220;
        B=clusterLeitungen(i,3)*Bb220;
        yij = (inv(R+1i*X))*clusterLeitungen(i,5)/ue^2;
        SLeitungr(i)=(abs((Uar(cluster(:,1)==clusterLeitungen(i,1))*220000.*exp(1i*dtar(cluster(:,1)==clusterLeitungen(i,1))))-(Uar(cluster(:,1)==clusterLeitungen(i,2))*220000.*exp(1i*dtar(cluster(:,1)==clusterLeitungen(i,2)))))^2)*conj(yij);
    elseif clusterLeitungen(i,4)==110
        R=clusterLeitungen(i,3)*Rb110;
        X=clusterLeitungen(i,3)*Xb110;
        B=clusterLeitungen(i,3)*Bb110;
        yij = (inv(R+1i*X))*clusterLeitungen(i,5)/ue2^2;
        SLeitungr(i)=(abs((Uar(cluster(:,1)==clusterLeitungen(i,1))*110000*exp(1i*dtar(cluster(:,1)==clusterLeitungen(i,1))))-(Uar(cluster(:,1)==clusterLeitungen(i,2))*110000.*exp(1i*dtar(cluster(:,1)==clusterLeitungen(i,2)))))^2)*conj(yij);
    end
end
