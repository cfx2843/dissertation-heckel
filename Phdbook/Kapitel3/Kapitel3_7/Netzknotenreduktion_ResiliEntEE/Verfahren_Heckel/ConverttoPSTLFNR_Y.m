function [Pa,Qa,Ua,DTAa] = ConverttoPSTLFNR_Y(Leitungen,Knoten,AnzL,AnzK,clusterKnoten,cluster)
...
cluster_mitKnoteninfo=[cluster,clusterKnoten(:,2)];

cluster_sort=sortrows(cluster_mitKnoteninfo,AnzK+1,'descend');

cluster_sort(:,AnzK+1)=[];

Knotensort=sortrows(Knoten,2,'descend');

clusterKnotensort=sortrows(clusterKnoten,2,'descend');

Leitungenneuenum=Leitungen;

AnzCluster=length(clusterKnoten);
...
for i=1:AnzCluster
    if clusterKnotensort(i,2)==2
        knotend(i,2)=1;
        knotend(i,3)=-clusterKnotensort(i,6)/Sb;
        knotend(i,4)=-clusterKnotensort(i,7)/Sb;
        knotend(i,7)=-inf;
        knotend(i,8)=inf;
        knotend(i,9)=0.001;
        knotend(i,10)=70;   
        ...
        knotend(i,5)=clusterKnotensort(i,6)/Sb;
        knotend(i,6)=clusterKnotensort(i,7)/Sb;
    end
end
...
[Pa,Qa,Ua,DTAa,~,~,~,~] =...
pst_lfnr_Y(Pg,Qg,Pl,Ql,Emin,Emax,dmax,xd,U,ltd,ttd,ltdrossel,k,pu,Err,0,cluster_sort,AnzCluster);

%Ausgabe-Größen zurück sortieren

Pa=[Pa(1:AnzCluster),cluster_sort(:,1)];

Pa=sortrows(Pa,2,'ascend');

Pa(:,2)=[];

Qa=[Qa(1:AnzCluster),cluster_sort(:,1)];
...
Ua=[Ua(1:AnzCluster),cluster_sort(:,1)];
...
DTAa=[DTAa(1:AnzCluster),cluster_sort(:,1)];
