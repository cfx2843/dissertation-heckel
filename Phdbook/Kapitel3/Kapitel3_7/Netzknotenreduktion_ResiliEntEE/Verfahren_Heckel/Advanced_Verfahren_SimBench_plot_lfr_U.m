...
erelu=zeros(length(zielAnzvec),1);
ereld=zeros(length(zielAnzvec),1);
zaehlplot=1;
for zielAnz=210:5:600
    %Die Clustermatrix wird erstellt
    %Die Claustermatrix beinhaltet die Zuordnung der Knoten zu Clustern. Jeder
    %Knoten behält seine Nummer
    cluster=zeros(AnzK,AnzK);
    ...
    %Nun muss der Fehler berechnet werden. Dafür werden nur die
    %Spannungswerte an den Knoten, die Cluster bilden benötigt
    ...
    Uf=abs(Uacluster-Uar);
    
    Df=abs(Dtacluster-Dtar);
    
    eu = norm(Uf,'fro');
    ed = norm(Df,'fro');
    
    erelu(zaehlplot)=eu/(norm(Uacluster,'fro'));
    ereld(zaehlplot)=ed/(norm(Dtacluster,'fro'));
...
figure;
hold on;
plot(zielAnzvec,erelu,'b');
plot(zielAnzvec,ereld,'r');
hold off;
