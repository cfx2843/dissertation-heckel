...
for i=1:Anzpp
    ...
    for j=1:AnzK
        if isequal(stringpp,char(charknotennummer{j}))
            indexpp=j; %hier gibt es noch ein Problem
        end
    end
...
%Leistungen veärndern für Konvergenz

% Knoten(:,7:8)=Knoten(:,7:8)*0.65;

konvfehlerbasis=0;

[Pa,Qa,Ua,dta,konvfehlerbasis] = ConverttoPSTLFNR_SimBench(Leitungen,Knoten,AnzL,AnzK);
...
zaehlplot=1;
konvfehlerspeicher=zeros(length(zielAnzvec),1);
for zielAnz=5:1:600
...
% zielAnz=60;
    ...
    [Par,Qar,Uar,dtar,konvfehler] = ConverttoPSTLFNR_SimBench(clusterLeitungen,clusterKnoten,AnzClusterLeitungen,AnzCluster);
    
    konvfehlerspeicher(zaehlplot)=konvfehler;
    
    [zielAnzvec(zaehlplot),konvfehler]
