...
for zielAnz=5:5:600
    ...
    %leere Einträge von clusterLeitungen löschen
    loesch=zeros(AnzL,1);
    for i=1:AnzL
    ...
    %Hier ist nun Platz für die Optimierung
    %OPTIMIERUNG
    
    
    %Die Netzgleichungen müssen nun mittels Schleifeaugebaut werden
    %Dafür muss zunächst die Knotenadmittanzmatrix aufgebaut werden
    ...
    for lauf=1:AnzClusterLeitungen
        ...
        if clusterLeitungenneuenum(lauf,4)==380  %Admittanzmatrix ist auf 380kV bezogen
            yij = sprintf('((inv(%d*xopt(%d)+1i*%d*xopt(%d)))*%d)',Rb,lauf,Xb,lauf,clusterLeitungenneuenum(lauf,5));
            yii = sprintf('(1i*0.5*%d*xopt(%d)*%d)',Bb,lauf,clusterLeitungenneuenum(lauf,5));
            
        elseif clusterLeitungenneuenum(lauf,4)==220
            yij = sprintf('(((inv(%d*xopt(%d)+1i*%d*xopt(%d)))*%d)/%d)',Rb,lauf,Xb,lauf,clusterLeitungenneuenum(lauf,5),ue^2);
            yii = sprintf('((1i*0.5*%d*xopt(%d)*%d)/%d)',Bb,lauf,clusterLeitungenneuenum(lauf,5),ue^2);
        elseif clusterLeitungenneuenum(lauf,4)==110
            yij = sprintf('(((inv(%d*xopt(%d)+1i*%d*xopt(%d)))*%d)/%d)',Rb,lauf,Xb,lauf,clusterLeitungenneuenum(lauf,5),ue2^2);
            yii = sprintf('((1i*0.5*%d*xopt(%d)*%d)/%d)',Bb,lauf,clusterLeitungenneuenum(lauf,5),ue2^2);
        end
    ...
    %Jetzt müssen auch die Knotengleichungen mittels Schleife aufgebaut werden    
    ...
    %Zusammensetzen des Strings für die Nebenbindung
    ...
    for i=1:AnzCluster
        ...
        if(clusterKnotenneuenum(i,2)==2) %Erzeugungsknoten
            if slackset==0 %Referenzknoten
                ...
                for j=1:AnzCluster
                    ...
                    if j==i
                        ...
                        partsumstring=sprintf('xopt(%d)*exp(-(xopt(%d)))*',AnzClusterLeitungen+j,AnzClusterLeitungen+AnzCluster+j);
                ...
                Sergstring=sprintf('%d+1i*xopt(%d)',AnzClusterLeitungen+2*AnzCluster+zaehlerz,AnzClusterLeitungen+2*AnzCluster+zaehlerz+1);
                ...
                SGleichungenstring{i}=sprintf('1*exp(xopt(%d))*',AnzClusterLeitungen+AnzCluster+i);
                ...
                for j=1:AnzCluster
                    ...
                    if j==i
                        ...
                        partsumstring=sprintf('1*exp((-xopt(%d)))*',AnzClusterLeitungen+AnzCluster+j);
                ...
                Sergstring=sprintf('%d+1i*xopt(%d)',-clusterKnotenneuenum(i,7)/Sb,AnzClusterLeitungen+2*AnzCluster+zaehlerz);
            ...
            SGleichungenstring{i}=sprintf('xopt(%d)*exp(xopt(%d))*',AnzClusterLeitungen+i,AnzClusterLeitungen+AnzCluster+i);
    ...
    %String für die Zielfunktion zusammensetzen
    ...
    for i=1:AnzCluster
        ...
        UGleichungenstring{i}=sprintf('%d-xopt(%d)',Uacluster(+i),AnzClusterLeitungen+i);
    ...
    for i=1:AnzCluster
        ...
        UGleichungenstring{i}=sprintf('%d-xopt(%d)',Dtacluster(i),AnzClusterLeitungen+AnzCluster+i);
    ...
    x0=[clusterLeitungenneuenum(:,3);Uacluster;Dtacluster;Sinitial];
    
    zielfun=str2func(zielfunstring);
    
    lb=[ones(AnzClusterLeitungen,1);zeros(AnzCluster,1);-10*ones(AnzCluster,1);-10000*abs(Sinitial)];
    
    ub=[300*ones(AnzClusterLeitungen,1);10*ones(AnzCluster,1);10*ones(AnzCluster,1);10000*abs(Sinitial)];
