...
for lauf=start:step:ende
    ...
    [clusterKnoten,clusterLeitungen,erelp,erelq,Parslack,Qarslack,slacknumber]=Advanced_Verfahren_SimBench_refbus_function(lauf);
    
    [clusterKnotenNEP,clusterLeitungenNEP,erelpNEP,erelqNEP,ParslackNEP,QarslackNEP,slacknumberNEP]=Advanced_Verfahren_SimBench_NEP_refbus_function(lauf);
...
%     abwrelq(zaehlplot)=abwq/(norm(SLeitungr,'fro'));
    ...
    verlPNEP=ParslackNEP*Sb+clusterKnotenNEP(slacknumberNEP,7);
     ...
     abverlp(zaehlplot)=(verlP-verlPNEP)/abs(verlP);%abs(verlP-verlPNEP)/abs(verlP);
     abverlq(zaehlplot)=(verlQ-verlQNEP)/abs(verlQ);%abs(verlQ-verlQNEP)/abs(verlQ);
