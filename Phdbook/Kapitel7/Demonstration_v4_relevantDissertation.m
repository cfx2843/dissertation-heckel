...
%     p(i)=1;% anzahl der Leitungen die wegfallen
%     t(i)=40;% Zeitpunkt
% end
% for i=1:brokenlines
...
if sum1 <0 % erstellen des neuen last knoten der an den betragsmäßig größsten knoten verbunden wird
    knoten(nk+1,:)=knoten(1,:);
    knoten(nk+1,1)=nk+1;
    knoten(nk+1,2)=1;
    knoten(nk+1,6)=knoten(b,6);
    knoten(nk+1,7)=-sum1-sum2/2;
    knoten(nk+1,8)=0;
    nk=nk+1;
    clusterLeitungen(nw+1,:)=[b, nk, 1, knoten(b,6), 100, 11]; %erstellen einer neuen Leitung zum Lastknoten
end
clusterLeitungen2=clusterLeitungen;
...
%%  Trafos einfügen
% x*1000+1 Formel für neue verbindung
    ...
    for i=1:nk
        for j=1:nw
            if clusterBus(i,1)==wire(j,k) && clusterBus(i,2)~=wire(j,4)
                ...
                clusterIdleTrafo(m,2)=clusterBus(i,1)*1000+m;
                clusterIdleTrafo(m,3)=clusterBus(i,2);
                clusterIdleTrafo(m,4)=wire(j,4);
                wire(j,k)=clusterBus(i,1)*1000+m;
                m=m+1;
            end
        end
    end
...
%% dummy für zuhause
%  ModelFile=['E:/TUHH_Projekt/MM/Nachbau.mo'];
%  Modelname= 'Nachbau'; %Modell name
%  fileID=fopen('E:/TUHH_Projekt/MM/Nachbau.mo','w');
%%
...
fprintf(fileID,['//wire\n']); 
for i=1:nw  
    fprintf(fileID,[' TransiEnt.Components.Electrical.Grid.PiModelComplex_advanced Path_',num2str(wire(i,1)),'_',num2str(wire(i,2)),'_',num2str(wire(i,6)),'(\n',...
...
for i=1:nb
    ...
    fprintf(fileID,['TransiEnt.Components.Electrical.Grid.ElectricBusbarComplex Bus_',num2str(numbervec2(i)),' (port=',num2str(port(i)),')',... 
...
for i=1:nt
	...
	fprintf(fileID,['TransiEnt.Components.Electrical.PowerTransformation.OLTC.TapChangerController OLTC_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'( \n   numberTaps=11,    v_prim_n(displayUnit="kV") = ',num2str(trafo(i,3)),'e3,',... 
...
%Störung in Form der linear ansteigenden Blindlast

fprintf(fileID,['TransiEnt.Components.Boundaries.Electrical.ComplexPower.PQBoundary pQBoundary_',num2str(indexMVSI),'( \n useInputConnectorP=false, v_n=', num2str(load(indexMVSI,6)*10^3),') annotation(Placement(transformation(extent={{170,134},{190,154}}))); \n \n']);

fprintf(fileID,['Modelica.Blocks.Sources.Ramp ramp( \n  height=',num2str(-1.1*critQ),', duration=380, startTime=10) annotation (Placement(transformation(extent={{98,230},{118,250}})));']);
    ...
    while j<=port(i)   	% anschluesse an ports
        ...
        for k=1:nw 		%check wire
            if numbervec2(i)==wire(k,1)
                dlmwrite(ModelFile,['connect(Bus_',num2str(numbervec2(i)),'.epp_[',num2str(j),'],Path_',num2str(wire(k,1)),'_',num2str(wire(k,2)),'_',num2str(wire(k,6)),'.epp_n);'],'-append','delimiter','');
                j=j+1;
            elseif numbervec2(i)==wire(k,2)
                dlmwrite(ModelFile,['connect(Bus_',num2str(numbervec2(i)),'.epp_[',num2str(j),'],Path_',num2str(wire(k,1)),'_',num2str(wire(k,2)),'_',num2str(wire(k,6)),'.epp_p);'],'-append','delimiter','');
                j=j+1;
            end
        end
		...
		for k=1:wireBrook 		%check brook wire
            if numbervec2(i)==clusterLeitungBrook(k,1)
                dlmwrite(ModelFile,['connect(Bus_',num2str(numbervec2(i)),'.epp_[',num2str(j),'],FPath_',num2str(clusterLeitungBrook(k,1)),'_',num2str(clusterLeitungBrook(k,2)),'_',num2str(clusterLeitungBrook(k,6)),'.epp_n);'],'-append','delimiter','');
                j=j+1;
            elseif numbervec2(i)==clusterLeitungBrook(k,2)
                dlmwrite(ModelFile,['connect(Bus_',num2str(numbervec2(i)),'.epp_[',num2str(j),'],FPath_',num2str(clusterLeitungBrook(k,1)),'_',num2str(clusterLeitungBrook(k,2)),'_',num2str(clusterLeitungBrook(k,6)),'.epp_p);'],'-append','delimiter','');
                j=j+1;
            end
        end
    end
...
for i=1:nt
    for j=1:nw
        ...
        if    trafo(i,1)==wire(j,1)&&trafo(i,1)==numbervec(k)
            dlmwrite(ModelFile,['connect(sT_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.epp_p,  Path_',num2str(wire(j,1)),'_',num2str(wire(j,2)),'_',num2str(wire(j,6)),'.epp_n);'],'-append','delimiter','');
			dlmwrite(ModelFile,['connect(sT_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.ratio_set, OLTC_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.RatioOut);'],'-append','delimiter','');
			dlmwrite(ModelFile,['connect(Path_',num2str(wire(j,1)),'_',num2str(wire(j,2)),'_',num2str(wire(j,6)),'.epp_n, OLTC_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.epp);'],'-append','delimiter','');
        elseif trafo(i,1)==wire(j,2) &&trafo(i,1)==numbervec(k)
            dlmwrite(ModelFile,['connect(sT_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.epp_p, Path_',num2str(wire(j,1)),'_',num2str(wire(j,2)),'_',num2str(wire(j,6)),'.epp_p);'],'-append','delimiter','');
			dlmwrite(ModelFile,['connect(sT_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.ratio_set, OLTC_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.RatioOut);'],'-append','delimiter','');
			dlmwrite(ModelFile,['connect(Path_',num2str(wire(j,1)),'_',num2str(wire(j,2)),'_',num2str(wire(j,6)),'.epp_p, OLTC_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.epp);'],'-append','delimiter','');
        elseif trafo(i,2)==wire(j,1) &&trafo(i,2)==numbervec(k)
            dlmwrite(ModelFile,['connect(sT_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.epp_n, Path_',num2str(wire(j,1)),'_',num2str(wire(j,2)),'_',num2str(wire(j,6)),'.epp_n);'],'-append','delimiter','');
			dlmwrite(ModelFile,['connect(sT_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.ratio_set, OLTC_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.RatioOut);'],'-append','delimiter','');
			dlmwrite(ModelFile,['connect(Path_',num2str(wire(j,1)),'_',num2str(wire(j,2)),'_',num2str(wire(j,6)),'.epp_n, OLTC_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.epp);'],'-append','delimiter','');
        elseif trafo(i,2)==wire(j,2) &&trafo(i,2)==numbervec(k)
            dlmwrite(ModelFile,['connect(sT_',num2str(trafo(i,1)),'_',num2str(trafo(i,2)),'.epp_n, Path_',num2str(wire(j,1)),'_',num2str(wire(j,2)),'_',num2str(wire(j,6)),'.epp_p);'],'-append','delimiter','');
    ...
    for j=1:wireBrook
        for k=1:trb
            if    clusterITBrook(i,1)==clusterLeitungBrook(j,1)&&clusterITBrook(i,1)==numvec_brook(k)
                dlmwrite(ModelFile,['connect(sTb_',num2str(clusterITBrook(i,1)),'_',num2str(clusterITBrook(i,2)),'.epp_p,  FPath_',num2str(clusterLeitungBrook(j,1)),'_',num2str(clusterLeitungBrook(j,2)),'_',num2str(clusterLeitungBrook(j,6)),'.epp_n);'],'-append','delimiter','');
                ...
                dlmwrite(ModelFile,['connect(FPath_',num2str(clusterLeitungBrook(j,1)),'_',num2str(clusterLeitungBrook(j,2)),'_',num2str(clusterLeitungBrook(j,6)),'.epp_n, OLTCb_',num2str(clusterITBrook(i)),'_',num2str(clusterITBrook(i,2)),'.epp);'],'-append','delimiter','');
            elseif clusterITBrook(i,1)==clusterLeitungBrook(j,2) &&clusterITBrook(i,1)==numvec_brook(k)
                dlmwrite(ModelFile,['connect(sTb_',num2str(clusterITBrook(i,1)),'_',num2str(clusterITBrook(i,2)),'.epp_p, FPath_',num2str(clusterLeitungBrook(j,1)),'_',num2str(clusterLeitungBrook(j,2)),'_',num2str(clusterLeitungBrook(j,6)),'.epp_p);'],'-append','delimiter','');
                dlmwrite(ModelFile,['connect(sTb_',num2str(clusterITBrook(i,1)),'_',num2str(clusterITBrook(i,2)),'.ratio_set, OLTCb_',num2str(clusterITBrook(i)),'_',num2str(clusterITBrook(i,2)),'.RatioOut);'],'-append','delimiter','');
                dlmwrite(ModelFile,['connect(FPath_',num2str(clusterLeitungBrook(j,1)),'_',num2str(clusterLeitungBrook(j,2)),'_',num2str(clusterLeitungBrook(j,6)),'.epp_p, OLTCb_',num2str(clusterITBrook(i)),'_',num2str(clusterITBrook(i,2)),'.epp);'],'-append','delimiter','');
            elseif clusterITBrook(i,2)==clusterLeitungBrook(j,1) &&clusterITBrook(i,2)==numvec_brook(k)
                dlmwrite(ModelFile,['connect(sTb_',num2str(clusterITBrook(i,1)),'_',num2str(clusterITBrook(i,2)),'.epp_n, FPath_',num2str(clusterLeitungBrook(j,1)),'_',num2str(clusterLeitungBrook(j,2)),'_',num2str(clusterLeitungBrook(j,6)),'.epp_n);'],'-append','delimiter','');
                dlmwrite(ModelFile,['connect(sTb_',num2str(clusterITBrook(i,1)),'_',num2str(clusterITBrook(i,2)),'.ratio_set, OLTCb_',num2str(clusterITBrook(i)),'_',num2str(clusterITBrook(i,2)),'.RatioOut);'],'-append','delimiter','');
                dlmwrite(ModelFile,['connect(FPath_',num2str(clusterLeitungBrook(j,1)),'_',num2str(clusterLeitungBrook(j,2)),'_',num2str(clusterLeitungBrook(j,6)),'.epp_n, OLTCb_',num2str(clusterITBrook(i)),'_',num2str(clusterITBrook(i,2)),'.epp);'],'-append','delimiter','');
            elseif clusterITBrook(i,2)==clusterLeitungBrook(j,2) && clusterITBrook(i,2)==numvec_brook(k)
                dlmwrite(ModelFile,['connect(sTb_',num2str(clusterITBrook(i,1)),'_',num2str(clusterITBrook(i,2)),'.epp_n, FPath_',num2str(clusterLeitungBrook(j,1)),'_',num2str(clusterLeitungBrook(j,2)),'_',num2str(clusterLeitungBrook(j,6)),'.epp_p);'],'-append','delimiter','');
                dlmwrite(ModelFile,['connect(sTb_',num2str(clusterITBrook(i,1)),'_',num2str(clusterITBrook(i,2)),'.ratio_set, OLTCb_',num2str(clusterITBrook(i)),'_',num2str(clusterITBrook(i,2)),'.RatioOut);'],'-append','delimiter','');
                dlmwrite(ModelFile,['connect(FPath_',num2str(clusterLeitungBrook(j,1)),'_',num2str(clusterLeitungBrook(j,2)),'_',num2str(clusterLeitungBrook(j,6)),'.epp_p, OLTCb_',num2str(clusterITBrook(i)),'_',num2str(clusterITBrook(i,2)),'.epp);'],'-append','delimiter','');
            end
        end
...
for i=1:wireBrook 
	dlmwrite(ModelFile,['connect(Step_',num2str(i),'.y,FPath_',num2str(clusterLeitungBrook(i,1)),'_',num2str(clusterLeitungBrook(i,2)),'_',num2str(clusterLeitungBrook(i,6)),'.switched_input);'],'-append','delimiter',''); 
end
for i=1:loadjump
...
dlmwrite(ModelFile,['connect(pQBoundary_',num2str(indexMVSI),'.epp,Load_',num2str(load(indexMVSI,1)),'.epp);'],'-append','delimiter',''); 
dlmwrite(ModelFile,['connect(ramp.y,pQBoundary_',num2str(indexMVSI),'.Q_el_set);'],'-append','delimiter',''); 
