...
model ClaRa_HeatpumpCharline_mitSpeicher_Real_Test
 ...
 TransiEnt.Basics.Blocks.LimPID PID_TWV(
  ...
  TransiEnt.Consumer.Heat.ThermalHeatConsumer_L3.ThermalHeatConsumer_L3 thermalHeatConsumer_CHP(
    ...
    redeclare TransiEnt.Consumer.Heat.ThermalHeatConsumer_L3.Base.ConstructionData.EnEV_Roof matLayRoof) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-51,52})));
  Modelica.Blocks.Sources.Constant ScaleFactor(k=scalingFactor*34000)           annotation (Placement(transformation(extent={{-54,-62},{-34,-42}})));
  TransiEnt.Components.Boundaries.Electrical.ComplexPower.SlackBoundary
                                                                   electricGrid(f_n=50)                  annotation (Placement(transformation(extent={{178,38},{198,58}})));
  ...
  Modelica.Blocks.Sources.RealExpression targetsupplyTemperature_boiler3(y=heatPumpElectricCharlineDG_real_testinertia.Q_flow_set_.y) annotation (Placement(transformation(extent={{56,-84},{86,-68}})));
  Phdbook.Kapitel4.Kapitel4_1.Components.PumpControl pumpControl(
    c_p=4200,
  ...
  Phdbook.Kapitel4.Kapitel4_1.Components.HeatPumpElectricCharlineDG_real heatPumpElectricCharlineDG_real_testinertia(
    Q_flow_n=ScaleFactor.k*1.8,
    heatFlowBoundary(change_sign=true),
    usePowerPort=true,
    redeclare TransiEnt.Basics.Interfaces.Electrical.ComplexPowerPort epp,
    MechanicalBoundary1(change_sign=true)) annotation (Placement(transformation(extent={{180,-6},{200,14}})));
  ...
  Modelica.Blocks.Sources.RealExpression input_set_temperature_storage1(y=273.15 + 65) annotation (Placement(transformation(extent={{32,60},{52,84}})));
  Phdbook.Kapitel4.Kapitel4_1.Components.LimPIDDG PID_CHP1(
    ...
    ramp(
      ...
      duration=100000,
      offset=5,
      startTime=2010000)) "Change of Ramp parameters necessary, if the start time is different." annotation (Placement(transformation(extent={{76,60},{96,80}})));
equation
  ...
  connect(heatPumpElectricCharlineDG_real_testinertia.waterPortOut, tempBeforeCons1.port) annotation (Line(
      points={{190.769,-6},{190.769,-14},{138,-14},{138,28},{146,28}},
      color={175,0,0},
      thickness=0.5));
  connect(heatPumpElectricCharlineDG_real_testinertia.waterPortIn, tempBeforeCons2.port) annotation (Line(
      points={{184.615,-6},{184.615,-32},{174,-32},{174,-60},{182,-60}},
      color={175,0,0},
      thickness=0.5));
  connect(heatPumpElectricCharlineDG_real_testinertia.epp, electricGrid.epp) annotation (Line(
      points={{195.385,4},{202,4},{202,22},{178,22},{178,48}},
      color={28,108,200},
      thickness=0.5));
  connect(input_set_temperature_storage1.y, PID_CHP1.u_s) annotation (Line(points={{53,72},{64,72},{64,70},{74,70}}, color={0,0,127}));
  connect(meas_temperature_storage1.y, PID_CHP1.u_m) annotation (Line(points={{55,53.5},{86.1,53.5},{86.1,58}}, color={0,0,127}));
  connect(PID_CHP1.y, heatPumpElectricCharlineDG_real_testinertia.Q_flow_set) annotation (Line(points={{97,70},{128,70},{128,66},{162,66},{162,4},{180,4}}, color={0,0,127}));
...
end ClaRa_HeatpumpCharline_mitSpeicher_Real_Test;
