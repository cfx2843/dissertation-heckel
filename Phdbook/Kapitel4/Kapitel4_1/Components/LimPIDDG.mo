...
block LimPIDDG "P, PI, PD, and PID controller with limited output, anti-windup compensation and delayed, smooth activation (if wanted)"
...
//Time Resononse of the Controller -------
  ...
  parameter Modelica.Units.SI.Time Tau_i(min=Modelica.Constants.small)=0.5 "1/Ti is gain of integrator block"
                                      annotation(Dialog(enable=controllerType==Modelica.Blocks.Types.SimpleController.PI or
                                controllerType==Modelica.Blocks.Types.SimpleController.PID,group="Time Response of the Controller"));
 parameter Modelica.Units.SI.Time Tau_d(min=0)=0.1 "Gain of derivative block"
                              annotation(Dialog(enable=controllerType==Modelica.Blocks.Types.SimpleController.PD or
                                controllerType==Modelica.Blocks.Types.SimpleController.PID,group="Time Response of the Controller"));

  parameter Modelica.Units.SI.Time Ni(min=100*Modelica.Constants.eps) = 0.9 "1/Ni is gain of anti-windup compensation"
...
public
  ...
  Modelica.Blocks.Math.Gain P(k=k) annotation (Placement(transformation(extent={{-42,56},{-22,76}}, rotation=0)));
...
Modelica.Blocks.Logical.GreaterThreshold I_activation(threshold=Tau_lag_I) if with_I
  ...
  Modelica.Blocks.Sources.Ramp ramp(
    ...
    duration=250000,
    offset=5,
    startTime=2010000) annotation (Placement(transformation(extent={{-70,90},{-50,110}})));
  Modelica.Blocks.Math.Product product1 annotation (Placement(transformation(extent={{-2,74},{18,94}})));
initial equation
  if use_reset then
...
equation
 ...
 if initOption==503 and limitsAtInit and (y_start < y_min or y_start > y_max) then
  ...
  connect(resetP.y, P.u) annotation (Line(points={{-78,59.5},{-78,66},{-44,66}}, color={0,0,127}));
...
connect(I_activation.y, switch_OnOff_I.u2) annotation (Line(points={{-152,-77},{-152,-81.5},{-50.1,-81.5}}, color={255,0,255}));
  connect(P.y, product1.u2) annotation (Line(points={{-21,66},{-8,66},{-8,78},{-4,78}}, color={0,0,127}));
  connect(ramp.y, product1.u1) annotation (Line(points={{-49,100},{-20,100},{-20,90},{-4,90}}, color={0,0,127}));
  connect(product1.y, addPID.u1) annotation (Line(points={{19,84},{26,84},{26,4},{41,4}}, color={0,0,127}));
  ...
  <li> 25.11.09 Update to independently set the gain and time constants of the PID, added a new parameter \"sign\" for case dependent control error evaluation, Friedrich Gottelt, XRG Simulation GmbH</li>
    ...
    Diagram(coordinateSystem(
        ...
        initialScale=0.1), graphics={Text(
          extent={{26,-126},{270,-216}},
          lineColor={28,108,200},
          textString="Multiplikation von k mit einer Rampenfunktion zur Vereinfachung der Initialisierung.
Bei Änderungen der Startzeiten müssen die Parameter der Rampenfunktion angepasst werden.")}));
end LimPIDDG;
