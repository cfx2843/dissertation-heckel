...
model ClaRa_HeatpumpCharline_Speicher_Real
...
public
  ...
  HeatPumpElectricCharlineDG_real heatPumpElectricCharlineDG_real_testinertia(
    Q_flow_n=ScaleFactor.k*1.8,
    heatFlowBoundary(change_sign=true),
    usePowerPort=true,
    redeclare TransiEnt.Basics.Interfaces.Electrical.ComplexPowerPort epp) annotation (Placement(transformation(extent={{180,-6},{200,14}})));
...
protected
  Phdbook.Kapitel4.Kapitel4_1.Components.LimPIDDG PID_CHP1(
    ...
    y_inactive=0,
    initOption=503,
    y_start=1) "It is necessary to change the ramp parameters, if a different start time is chosen." annotation (Placement(transformation(extent={{76,60},{96,80}})));
public
  TransiEnt.Basics.Interfaces.Electrical.ComplexPowerPort epp annotation (Placement(transformation(extent={{-84,-98},{-64,-78}})));
equation
  ...
  connect(heatPumpElectricCharlineDG_real_testinertia.waterPortIn, tempBeforeCons2.port) annotation (Line(
      ...
      color={175,0,0},
      thickness=0.5));
  connect(set_temperature_storage1.y, PID_CHP1.u_s) annotation (Line(points={{53,72},{64,72},{64,70},{74,70}}, color={0,0,127}));
  ...
  connect(heatPumpElectricCharlineDG_real_testinertia.epp, epp) annotation (Line(
      points={{195.385,4},{198,4},{198,-88},{-74,-88}},
      color={28,108,200},
      thickness=0.5));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-80,-120},{200,100}})),
                                                                 Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-80,-120},{200,100}})),
    experiment(
      StartTime=2000000,
      StopTime=2401500,
      Interval=100,
      __Dymola_Algorithm="Dassl"));
end ClaRa_HeatpumpCharline_Speicher_Real;
